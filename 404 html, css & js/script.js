...
cursorEffect.forEach((e) => {
  e.addEventListener(
    "mouseenter",
    () => (cursor.style.animation = `blink .3s both`)
  );
  e.addEventListener(
    "mouseleave",
    () => (cursor.style.animation = `blink-reverse .3s both`)
  );
});
// typing effect
function typingEffect(element, speed, delay) {
  setTimeout(() => {
    if (element.hasAttribute("data-typingEffect")) {
      return;
    }
...
function mouseParallaxTranslate(e, element, range) {
  ...
  element.style.transform = `translate(${mouseX * range}px, ${
    mouseY * range
  }px)`;
}
